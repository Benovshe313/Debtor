...
class Program1
{
    ...
    public static void Main()
    {
        ...
        var result2 = debtors.Where(debtor => {
            int age = dateTime.Year - debtor.BirthDay.Year;
            return age >= 26 && age < 36; }).ToList();
        ...
        //    3) Borcu 5000 - den cox olmayan borclulari cixartmag

        var result3 = debtors.Where(debtor => debtor.Debt <= 5000).ToList();

        foreach (var debtor in result3)
        {
            Console.WriteLine(debtor);
        }
        ...
        var result4 = debtors.Where(result => result.FullName.Length > 18 && result.Phone.Count<char>(count => count == '7') >= 2).ToList();

        foreach (var debtor in result4)
        {
            Console.WriteLine(debtor);
        }
        ...
        var result5 = debtors.Where(debtor => debtor.BirthDay.Month == 12 || debtor.BirthDay.Month == 1 || debtor.BirthDay.Month == 2).ToList();

        foreach (var debtor in result5)
        {
            Console.WriteLine(debtor);
        }
        ...
        var average = debtors.Average(debtor => debtor.Debt);
        var result6 = debtors.Where(debt => debt.Debt > average);
        
        var resultOf = result6.OrderBy(debtor => debtor.FullName.Split(' ').Last()).ThenByDescending(debtor => debtor.Debt).ToList();

        foreach (var debtor in resultOf)
        {
            Console.WriteLine(debtor);
        }
        ...
        var result7 = debtors.Where(debtor => !debtor.Phone.Contains('8')).ToList();

        foreach (var debtor in result7)
        {
            string[] splitName = debtor.FullName.Split(' ');
            string surname = splitName[splitName.Length - 1];
            int age = currentTime.Year - debtor.BirthDay.Year;
            Console.WriteLine($" Surname: {surname} Age: {age} Debt: {debtor.Debt}");
        }
        ...
        var result8 = debtors.Where(debtor => {
        string fullName = debtor.FullName.Replace(" ", "").ToLower();
        var group = fullName.GroupBy(g => g);
        return group.Any(group => group.Count() >= 3);
        }).OrderBy(debtor => debtor.FullName).ToList();

        foreach (var debtor in result8)
        {
            Console.WriteLine(debtor);
        }
        ...
        var count = debtors.GroupBy(debtor => debtor.BirthDay.Year)
            .Select(group => new { Year = group.Key, Count = group.Count()}).ToList();

        var maxCount = count.OrderByDescending(year => year.Count).First();

        Console.WriteLine(maxCount.Year);

        Console.WriteLine("Ex 10");
        //    10)Borcu en boyuk olan 5 borclunun siyahisini cixartmaq

        var result10 = debtors.OrderByDescending(debtor => debtor.Debt).Take(5).ToList();

        foreach (var debtor in result10)
        {
            Console.WriteLine(debtor);
        }
        ...
        int totalDebt = debtors.Sum(debtor => debtor.Debt);
        Console.WriteLine(totalDebt);

        Console.WriteLine("Ex 12");
        //    12)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq

        var result12 = debtors.Where(debtor => debtor.BirthDay >= new DateTime(1939, 9, 1)).ToList();

        foreach (var debtor in result12)
        {
            Console.WriteLine(debtor);
        }
        ...
        var result13 = debtors.Where(debtor =>
        {
            var num = debtor.Phone.Where(char.IsDigit).ToArray();
            return num.Distinct().Count() == num.Length;

        }).ToList();

       foreach(var debtor in result13)
       {
       ...
       DateTime dateTime1 = DateTime.Now;
        List<Debtor> canPay = new List<Debtor> {};
        foreach (var debtor in debtors)
        {
            DateTime nextBirth = new DateTime(dateTime1.Year, debtor.BirthDay.Month, debtor.BirthDay.Day);
            if (nextBirth < dateTime1)
            {
                nextBirth = nextBirth.AddYears(1);
            }
            int month = ((nextBirth.Year - dateTime1.Year) * 12) + (nextBirth.Month - dateTime1.Month);
            int pay = month * 500;
            if (pay >= debtor.Debt) canPay.Add(debtor);
        }
        foreach (var debtorInfo in canPay)
        {
            Console.WriteLine(debtorInfo);
        }
        ...
        var result15 = debtors.Where(smile => smile.FullName.Contains<char>('s') && smile.FullName.Contains<char>('m') && smile.FullName.Contains<char>('i') && smile.FullName.Contains<char>('l') && smile.FullName.Contains<char>('e')).ToList();

        foreach(var debtor in result15)
        {
            Console.WriteLine(debtor);
        }
